...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
	...
	<!-- ZONE -->
	<xsl:template match="tei:zone" mode="pro">
	...
	<xsl:template match="node()[@attachment]" mode="pro">
		<xsl:element name="div">
	...
	<xsl:template match="tei:lb" mode="pro">
		<xsl:choose>
	...
	<!-- Page break -->
	<xsl:template match="tei:pb" mode="pro">
		<xsl:copy-of select="."/>
	</xsl:template>
	...
	<!-- SUBST substitution -->
	<xsl:template match="tei:subst" mode="pro" priority="3">
		<xsl:element name="span">
			<xsl:attribute name="class" select="$ed_name3,name()" separator="-"/>
	...
	<!-- ADD Addition -->
	<xsl:template match="tei:add" mode="pro" priority="2">
		<xsl:element name="span">
			<xsl:attribute name="class" select="$ed_name3,name()" separator="-"/>
			<xsl:choose>
	...
	<!--EXPAN expansion 
		...
		ORIG Original form
	-->
	<xsl:template match="tei:expan|tei:damage|tei:ex|tei:corr|tei:reg|tei:orig|tei:abbr" mode="pro">
		<xsl:element name="span">
			<xsl:attribute name="class" select="$ed_name3,name()" separator="-"/>
			<xsl:apply-templates mode="#current"/> 
		</xsl:element>
	...
	<xsl:template match="tei:supplied" mode="pro">
		<xsl:element name="span">
			<xsl:attribute name="class" select="$ed_name3, name()" separator="-"/>
	...
	<xsl:template match="tei:seg" mode="pro">
		<xsl:element name="span">
			<xsl:choose>
	...
	<!-- HI Highlighted text -->
	<xsl:template match="tei:hi" mode="pro">
		<xsl:element name="span">
			<!-- Aggiungi il valore di @rend alla classe. Se in @rend è presente un '.' viene sostituito con un '_' -->
	...
	<xsl:template match="tei:gap" mode="pro">
		<xsl:element name="span">
			<xsl:attribute name="class" select="$ed_name3,name()" separator="-"/><xsl:apply-templates mode="#current"/>
		</xsl:element>
	</xsl:template>
	...
	<xsl:template match="node()[name()='div'][@id='areaAnnotations']" mode="pro">
		<xsl:copy-of select="." />
	</xsl:template>
	
	<xsl:template match="node()[name()='div'][@id='AnnMenu' or @class='AnnMenuItem']|node()[name()='div'][contains(@class, 'AnnSubmenu')]" mode="pro">
	...
	<xsl:template match="tei:ref" mode="pro">
		<xsl:choose>
	...
	<xsl:template match="tei:front" mode="pro">
		<!-- do nothing -->
	</xsl:template>
	
	<xsl:template match="tei:body" mode="pro">
	...
	<!-- DESC Desc-->
	<xsl:template match="tei:desc" mode="pro">
		<xsl:element name="span">
	...
	<xsl:template match="tei:note//tei:title" mode="pro">
		<xsl:element name="span">
	...
	<!-- EMPH emphasized  -->
	<xsl:template match="tei:emph" mode="pro">
		<xsl:element name="span">
	...
	<!-- TERM -->
	<xsl:template match="tei:term" mode="pro">
		<xsl:choose>
	...
	<!-- DATE -->
	<xsl:template match="tei:date" mode="pro">
		<xsl:choose>
	...
	<xsl:template match="tei:persName[starts-with(@ref,'#')]" mode="pro">
		<xsl:choose>
		...
		<xsl:choose>
			<xsl:when test="current()//tei:forename or current()//tei:surname or current()//tei:sex or current()//tei:occupation">
				...
				<xsl:if test="current()/tei:note and current()/tei:note != ''">
					<span class='small-note'><xsl:apply-templates select="tei:note" mode="pro"/></span>
				</xsl:if>
			</xsl:when>
			<xsl:otherwise>
				<xsl:element name="span">
	...
	<!-- MEASURE  -->
	<xsl:template match="tei:measure" mode="pro">
		<xsl:choose>
	...
	<!-- ROLE NAME -->
	<xsl:template match="tei:roleName" mode="pro">
		<xsl:element name="span">
	...
	<!-- ORG NAME  -->
	<xsl:template match="tei:orgName" mode="pro">
		<xsl:choose>
		...
		<xsl:choose>
			<xsl:when test="current()[@type] or current()[@subtype] or current()[@role] or current()/tei:orgName or current()/tei:desc">
				...
				<xsl:if test="current()/tei:desc and current()/tei:desc != ''">
					<span class='small-note'><xsl:apply-templates select="current()/tei:desc" mode="pro"/></span>
				</xsl:if>
	...
	<xsl:template match="tei:placeName[starts-with(@ref,'#')]" mode="pro">
		<xsl:choose>
	...
	<!-- PTR Pointer -->
	<xsl:template match="tei:ptr" mode="pro">
		<xsl:choose>
	...
	<!-- QUOTE Quotes -->
	<xsl:template match="tei:quote" mode="pro">
		<xsl:element name="span">
