...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
    ...
    <xd:doc type="stylesheet">
        <xd:short>
            EN: Templates used to critcess the TEI elements of the TRANSCR, LINKING and gaiji modules.
    ...
    <xsl:template match="tei:fw" mode="crit">
        <xsl:choose>
            <xsl:when test="@place='top-middle'">
                <xsl:element name="span">
                    <xsl:attribute name="class" select="$ed_name4,'center'" separator="-"/>
                    <xsl:apply-templates mode="#current"/> 
                </xsl:element>
            </xsl:when>
            <xsl:when test="@place='top-right'">
                <xsl:element name="span">
                    <xsl:attribute name="class" select="$ed_name4,'right'" separator="-"/>
                    <xsl:apply-templates mode="#current"/> 
                </xsl:element>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="tei:ab" mode="crit" priority="2">
        <xsl:element name="span">
