...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
	...
	<xsl:template match="tei:pb" mode="facs dipl" priority="3">
		...
		<xsl:for-each
			select="//tei:facsimile/tei:surface[substring(@xml:id, string-length(@xml:id)-3)=$n]">
			...
			<xsl:element name="div">
				...
				<xsl:for-each select="tei:zone">
					...
					<xsl:variable name="CurrClass">
						<xsl:value-of select="@rendition"/>
					</xsl:variable>
					<xsl:element name="div">
						<xsl:variable name="CurrRectId">
							<xsl:value-of select="@xml:id"/>
						</xsl:variable>
						<xsl:variable name="RectWidth" select="number(@lrx) - number(@ulx)"/>
						<xsl:variable name="RectHeight" select="number(@lry) - number(@uly)"/>
						<xsl:attribute name="id">Area_<xsl:value-of select="$CurrRectId"
							/></xsl:attribute>
						<xsl:attribute name="class">Area</xsl:attribute>
						<xsl:attribute name="onclick">ShowAnn('<xsl:value-of select="$CurrRectId"
							/>')</xsl:attribute>
						...
						<!--<xsl:if test="number($ImageScaleFactor) &lt; 1">
							<xsl:attribute name="ondblclick">ShowZoom(<xsl:value-of select="@ulx"
									/>,<xsl:value-of select="@uly"/>,<xsl:value-of
									select="$RectWidth"/>,<xsl:value-of select="$RectHeight"
								/>)</xsl:attribute>
						</xsl:if>-->
						<xsl:attribute name="onmouseover">Highlight('<xsl:value-of
								select="$CurrRectId"/>')</xsl:attribute>
						...
						<xsl:variable name="CurrStyle"> position: absolute; left: <xsl:value-of
							select="@ulx"/>px; top: <xsl:value-of select="@uly"/>px; width:
							<xsl:value-of select="$RectWidth"/>px; height: <xsl:value-of
								select="$RectHeight"/>px; <!-- Add the colour setting from the tagsDecl in the header. -->
							<xsl:value-of
								select="//tei:tagsDecl/tei:rendition[@xml:id=$CurrClass]/tei:code"
						...
						<xsl:attribute name="style">
							<xsl:value-of select="normalize-space($CurrStyle)"/>
						</xsl:attribute>
						...
						<!--<xsl:attribute name="title">
							<xsl:choose>
								<xsl:when test="//tei:div[@facs = concat('#', $CurrRectId)]">
									<xsl:value-of select="//tei:div[@facs = $CurrRectId]/tei:head"/>
								</xsl:when>
								<xsl:otherwise>
									<xsl:value-of
										select="//tei:div[@corresp = concat('#', $CurrRectId)]/tei:head"
									/>
								</xsl:otherwise>
							</xsl:choose>
							<xsl:if test="number($ImageScaleFactor) &lt; 1">(Double-click to
								zoom.)</xsl:if>
						</xsl:attribute>-->
			...
			<xsl:element name="div">
				...
				<xsl:if test="tei:zone[@rendition=$CurrCategory]">
					<!-- EN: Mod by JK -->
					<xsl:element name="div">
						<xsl:attribute name="class">AnnSubmenu_<xsl:value-of select="$CurrCategory"
							/></xsl:attribute>
							...
							<xsl:for-each select="tei:zone[@rendition=$CurrCategory]">
								...
								<xsl:for-each-group select="//tei:text//node()[name()=$start_split]/node()"
									...
									<xsl:if test="current()[translate(@facs, '#', '') = $CurrAnnId]">
										<xsl:element name="li">
											<xsl:attribute name="style"
												>list-style:none;</xsl:attribute>
											<xsl:attribute name="class">AnnMenuItem</xsl:attribute>
											<xsl:attribute name="id">MenuItem_<xsl:value-of
												select="$CurrAnnId"/></xsl:attribute>
											<xsl:attribute name="onclick">JumpTo('<xsl:value-of
												select="$CurrAnnId"/>')</xsl:attribute>
											<xsl:attribute name="onmouseover"
												>Highlight('<xsl:value-of select="$CurrAnnId"
												/>')</xsl:attribute>
											<xsl:attribute name="onmouseout"
												>UnHighlight()</xsl:attribute>
											<xsl:apply-templates
												select="current-group()[not(self::tei:lb|self::tei:pb)]"
												mode="#current">
