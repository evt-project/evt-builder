...
<xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
	...
	<!-- EN: Copy all -->
	<!-- IT: Copia tutto -->
	<xsl:template match="@*|node()" mode="split-1 splitZero">
	<!-- EN: To add a new level add here the mode decreased by one, f.i.: mode="split-6" -->
	<!-- IT: Per aggiungere un nuovo livello aggiungere qui il mode decrementato di 1 es: mode="split-6" -->
		<xsl:copy>
	...
	<xsl:variable name="step-1">
		<xsl:apply-templates select="//tei:TEI" mode="split-1"/>
	</xsl:variable>
	

	<!--
		EN: to add a new level modify the previous template as follows (increase the select value by one and add the corresponding variable):
		IT: per aggiungere un nuovo livello modificare il precedente template nel seguente modo (incrementare di 1 il valore del select e aggiungere la corrispondente variabile):
