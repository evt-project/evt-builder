	...
	<xsl:template match="/" priority="1">
		...
		<xsl:if test="tei:TEI/tei:text">
			...
			<xsl:if test="$list_term = true()">
				<xsl:result-document method="html" encoding="UTF-8" media-type="text/plain"
					byte-order-mark="yes" href="{$filePrefix}/data/output_data/liste/listTerm.html"
					indent="yes">
					<xsl:element name="div">
			...
			<xsl:if test="$list_gloss = true()">
				<xsl:result-document method="html" encoding="UTF-8" media-type="text/plain"
					byte-order-mark="yes" href="{$filePrefix}/data/output_data/liste/listGloss.html"
					indent="yes">
					<xsl:element name="div">
	...
	<xsl:template name="page">
		...
		<xsl:for-each select="$edition_array">
			<xsl:if test=". != ''">
				...
				<xsl:result-document method="html" encoding="UTF-8" media-type="text/plain"
					byte-order-mark="yes"
					href="{$filePrefix}/data/output_data/{$edition_current}/page_{$pb_n}_{$edition_current}.html"
					indent="yes">
	...
	<xsl:template name="edition_level">
		...
		<xsl:if test="$edition_pos = 1">
			<xsl:choose>
				...
				<xsl:when
					test="$root//tei:facsimile/tei:surface[translate(@corresp, '#', '') = replace($pb_n, '-front', '')]//tei:zone[@rendition = 'Line']">
					<!--<xsl:copy-of select="current-group()"/>-->
					<!-- <-use this to find split errors -->
					<xsl:variable name="text">
	...
	<xsl:template name="search_file">
		<xsl:if test="$edition_array[1] != ''">
			...
			<xsl:result-document method="text"
				...
				indent="no"> {"pages": [ <xsl:for-each-group
					...
					<xsl:choose>
						<xsl:when test="current-group()/(descendant-or-self::lb)">
							<xsl:for-each-group select="current-group()[not(self::pb)]"
								...
								<xsl:if
									...
									<xsl:variable name="current_text">
										<xsl:apply-templates
											select="current-group()[not(self::tei:lb)]" mode="dipl"
										/>
									</xsl:variable>
