...
//Strip off the first four characters (Ann_).
		...
		ITLon = true;
		if (SelectedArea) {
			JumpTo(SelectedArea.replace('Area_', ''));
			SelectedArea == null;
		}
	}
}
...
function ShowAnn(ItemId) {
	...
	if (TheMenuItem != null) {
		...
		//scroll it
		$('#text_cont').animate({
			scrollTop: TheMenuItem.offsetTop - 5
		}); //Add by JK for EVT-builder
...
function switchITL() {
	...
	if (!switchITL.hasClass('likeInactive')) {
		if ($('.secondary_toggler.active').length > 0) {
			closeSecondaryImageContentOpened('switchITL', false);
			if (ITLon) {
		...
		if (!$('#left_header').hasClass('menuClosed')) {
			$("#image_tool").removeClass('hidden').show();
		}
	}
}
...
function InitializeHS() {
	if (HSon == false) {
		...
		if ($('#switchHS i ').hasClass('fa-circle-o')) {
			$('#switchHS i ').removeClass('fa-circle-o').addClass('fa-dot-circle-o');
		}
	...
	if (HSon == true) {
		...
		for (i = 0; i < TheAnnotation.length; i++) {
			...
			if (parseInt(TheAnnotation.css('top')) + annH > bottLimit) {
				TheAnnotation.css({
					top: imgH - image_toolH - 10 - annH + 'px'
				}); //-$("#left_header").height() ?
			}
		}
	}
...
function switchHS() {
	closeSecondaryImageContentOpened('switchHS',true);
	if (HSon == false) {
		//if ((magnifierON==true)&&(bigImage==true)){magOn();}
		if (magnifierON) {
			magOn();
