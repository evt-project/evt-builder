...
function updateEntitiesFiltered(frame) {
    frame.find('.like_select.filter').find('.option.selected').removeClass('selected').trigger('click');
}
...
function closeListsBox(speed) {
    $('#lists_cont').removeClass('bottomBoxOpened').hide('slide', {
        direction: 'down'
    },
    'linear', speed);
    ...
    $('#text_cont').find('.selected_from_list').removeClass('selected_from_list');
}
...
function filterListElements(filter) {
    ...
    if ($('.ul_list:visible').find(listsElementsFiltered).length == 0) {
        if ($('.no_elements').length > 0) {
            ...
            $('<li />').addClass('list_element').addClass('no_elements').append("<span lang='def'>NO_ELEMENTS</span>").appendTo('.ul_list:visible');
            window.lang.run();
        }
    } else {
        if ($('.no_elements').length > 0) {
...
function showListElementOccurrences(elem, listName) {
    if ($(elem).hasClass('list_element_opened')) {
        ...
        $(elem).parents('.list').find('.list_element_opened').removeClass('list_element_opened').find('.fa-angle-down').removeClass('fa-angle-down').addClass('fa-angle-right');
        $(elem).addClass('list_element_opened').find('.fa').removeClass('fa-angle-right').addClass('fa-angle-down');
    }
}
...
function prepareOccurrencesList(elem, listName) {
    ...
    occ_ref = $('#list_' + listName).find('#occorrenze_' + listName).find("span[data-ref='" + list_ref + "']");
    if (occ_ref.length > 0) {
        occ_ref.each(function () {
            ...
            if (occElem.length > 0) {
                ...
                occ++;
                occElem.attr('data-occ', occ).attr('title', occ + " " + window.lang.convert('OCCURRENCES', window.lang.currentLang));
                $(this).remove();
            } else {
                $(this).attr('data-occ', '1').attr('title', "1 " + window.lang.convert('OCCURRENCE', window.lang.currentLang)).text(window.lang.convert('FOL', window.lang.currentLang) + " " + pb_n + " - " + window.lang.convert('DOC', window.lang.currentLang) + " " + doc_lab).click(function () {
                    goToOccurrencePage(this, pb, doc);
                }).detach().appendTo(list_occ);
            }
        });
    } else {
...
function scrollDownListContainer(speed) {
    ...
    $('#lists_cont').addClass('collapsed').animate({
        top: newTop + 'px'
    },
    speed);
}
...
function bindDocListSortSelectClick() {
 $(".docList_sort__attribute_select .option_container .option").click(function () {
        console.log($(this).attr('data-value')); // the selected options’s value
   if (! $(this).hasClass('selected')) {
   
   // Is not currently selected
   $(".docList_sort__attribute_select .option_container .option .selected").removeClass('selected');         
	 	$(this).addClass('selected');     
		
        var container = $("#ul_listDocument");
        var items = $(".list_element");
        var value = $(this).attr('data-value');
    
     if (value == "sort_date") {
     items.each(function() {
       // Convert the string in 'data-sort-date' attribute to a more
       // standardized date format
       var date = $(this).attr("data-sort-date").split("-");
       var standardDate = date[0]+" "+ date[1]+" "+ date[2];
       standardDate = new Date(standardDate).getTime();
       $(this).attr("data-sort-date", standardDate);
 
    });
    
    items.sort(function(a,b){
        a = parseFloat($(a).attr("data-sort-date"));
        b = parseFloat($(b).attr("data-sort-date"));
