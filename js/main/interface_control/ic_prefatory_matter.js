...
function toggleFront(front_cont) {
	if ($(front_cont).is(":visible")) {
		...
		$(front_cont).show('slide', { direction: 'up' }, 400, function () {
...
function show_regesto(regesto_container, regesto) {
	var height  = $('#central_wrapper').height();
	...
	var regestoCont = $(regesto_container);
	regestoCont
		.removeClass('hidden')
		.animate({
			'top': '0px',
			...
			regestoCont.removeAttr('style');
			if ($(regesto_cont).parents("div[id*='frame']").attr('id') == 'main_left_frame' &&
				$('#span_ee_select').find('.option') == 0) {
				regestoCont.find('.showHide_regesto').hide();
			} else {
				regestoCont.find('.showHide_regesto').removeAttr('style');
			}
		});
	$(regesto).show();
	regestoCont.scrollTop(0);
	regestoCont.parents("div[id*='frame']")
		.find('.like_select.filter').css('opacity', '.5').addClass('not_active');
}
...
function hide_regesto(regesto_cont, regesto) {
	...
	if ($(this).attr('id') == "hide_regesto-add") {
		switchReg = $("#switchReg-add");
	} else {
		switchReg = $("#switchReg");
	}
	if (!switchReg.hasClass('disabled')) {
		switchReg.toggleClass('active')
			.find('.fa')
...
function toggleReg(cont) {
	if ($(cont).is(":visible")) {
		$(cont).hide('slide', {
			...
			var ee_val;
			var thisFrame = $(cont).parents("div[id*='frame']");
			ee_val = thisFrame.find('.main_ee_select .label_selected').attr('data-value');
...
function bindMsDescBtnClick() {
	$('#switch_msDesc').unbind('click').click(function () {
		if (!$(this).hasClass('disabled')) {
			if ($(this).hasClass('active')) { // HIDE
				closeSecondaryImageContentOpened('msDescClick', true);
				$('#switch_msDesc').removeClass('active');
			} else {
				openSecondaryImageContent('msDesc_cont', '#switch_msDesc');
			}
		}
	});
}
